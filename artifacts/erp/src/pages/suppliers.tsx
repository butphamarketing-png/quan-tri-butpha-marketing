...
export default function Suppliers() {
  ...
  const { data, isLoading } = useListSuppliers({ search: search || undefined, page, limit: 20 });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      if (editing) { await updateMutation.mutateAsync({ id: editing, data: form }); toast({ title: "Cập nhật thành công" }); }
      else { await createMutation.mutateAsync({ data: form }); toast({ title: "Thêm nhà cung cấp thành công" }); }
  ...
  return (
    <div>
      <PageHeader title="Nhà Cung Cấp" subtitle="Quản lý danh sách nhà cung cấp" action={{ label: "Thêm NCC", onClick: () => { setEditing(null); setForm(emptyForm); setShowForm(true); } }} />
      ...
      <DataTable data={rows} columns={columns} total={data?.total ?? 0} page={page} limit={20} onPageChange={setPage} loading={isLoading} emptyMessage="Chưa có nhà cung cấp" />
      <Dialog open={showForm} onOpenChange={(o) => { if (!o) { setShowForm(false); setEditing(null); } }}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>{editing ? "Cập Nhật Nhà Cung Cấp" : "Thêm Nhà Cung Cấp"}</DialogTitle></DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2 space-y-1"><Label>Tên Nhà Cung Cấp *</Label><Input required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
